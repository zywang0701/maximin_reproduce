...
# To replicate the setting 1 in paper. Actually I failed to find the setting1 code.
#############################Do not modify below################################
# delta
delta = 0
...
mean.target = rep(0, p)
# coefs
perb.vec = c(1,1.125,1.25,1.5,3.75,4,5,7,10,12)
perb = perb.vec[setting]
Bs = matrix(0, p, L)
Bs = matrix(0, p, L)
Bs[1:10, 1] = seq(1:10)/40
Bs[11:20, 1] = -seq(1:10)/40
Bs[21, 1] = 0.2
Bs[22:23, 1] = 1
for(j in 2:L){
  Bs[1:10, j] = Bs[1:10, 1] + perb/sqrt(300)
  Bs[21, j] = 0.5
  Bs[22:23, j] = 0.2
}
# loading
...
for(i.sim in 1:nsim){
  if(i.sim%%10==1) print(paste("Running Simulation --", i.sim))
...
filename <- paste("Task10-Perb-Setting",setting,"-n",n,"-L2","-delta0","-p",p,"-round",round,".RData", sep="")
save.image(filename)
