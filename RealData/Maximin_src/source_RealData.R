MaximinRD <- function(Xlist, Ylist, loading, LFest, X.target=NULL, cov.target=NULL,
  ...
  ## centralize Xs
  # for(l in 1:L) Xlist[[l]] = scale(Xlist[[l]],center = TRUE, scale=FALSE)
  ...
  X.target = scale(X.target, center=TRUE, scale=FALSE)
  ...
  for(l in 1:L){
    ...
    col.norm = apply(X, MARGIN = 2, FUN=function(x) 1/sqrt(mean(x^2)))
    ...
    if(intercept){
      ...
      Coef.est[, l] = Coef.est[,l]*col.norm
