...
t1 = Sys.time()
for(i.sim in 1:nsim){
  if(i.sim %% 5 == 1) print(paste("At seed", seed, ">>>Running Simulation:", i.sim))
...
t2 = Sys.time()
time.mm = difftime(t2, t1, units="secs")

# save RData
filename <- paste("task7-Challenge-highd-Setting",setting,"-L",L,"-n",n,"-p",p,"-round",round,".RData", sep="")
save.image(filename)
